...
int main()
{
    ...
    auto const rollSlotOffset = SystemUpdateSlotOffset(
        391, 420, 0xf8, 0x18);
    assert(modifierSlotOffset.has_value());
    assert(*modifierSlotOffset == 419ULL * 0xf8ULL + 0x18ULL);
    assert(rollSlotOffset.has_value());
    assert(*rollSlotOffset == 391ULL * 0xf8ULL + 0x18ULL);
    assert(!SystemUpdateSlotOffset(420, 420, 0xf8, 0x18).has_value());
    assert(!SystemUpdateSlotOffset(0, 0, 0xf8, 0x18).has_value());
    assert(!SystemUpdateSlotOffset(0, 1, sizeof(std::uintptr_t), 1)
        .has_value());
    assert(!SystemUpdateSlotOffset(
        1,
        2,
        (std::numeric_limits<std::size_t>::max)(),
        1).has_value());

    constexpr std::uintptr_t modifierOriginal = 0x1000;
    constexpr std::uintptr_t rollOriginal = 0x2000;
    constexpr std::uintptr_t modifierHook = 0x3000;
    constexpr std::uintptr_t rollHook = 0x4000;
    assert(!InstalledSystemHookPointersMatch(
        modifierOriginal,
        rollOriginal,
        modifierHook,
        rollHook,
    ...
    assert(!InstalledSystemHookPointersMatch(
        ...
        0,
        rollOriginal));
    assert(!ShouldRetireInstalledWorld(
        true, 0x5000, 0));
    assert(!ShouldRetireInstalledWorld(
        true, 0x5000, 0x5000));
    assert(!ShouldRetireInstalledWorld(
    ...
    StableWorldCandidateGate startupGate;
    constexpr std::uintptr_t firstWorld = 0x0000012345600000ULL;
    constexpr std::uintptr_t secondWorld = 0x0000012345700000ULL;
    assert(!startupGate.Observe(firstWorld, false).ready);
    for (std::uint32_t sample = 1;
         sample < kWorldCandidateStableSamples;
         ++sample) {
        assert(!startupGate.Observe(firstWorld, true).ready);
    }
    auto stableWorld = startupGate.Observe(firstWorld, true);
    assert(stableWorld.ready);
    assert(stableWorld.value == firstWorld);
    assert(!startupGate.Observe(secondWorld, true).ready);
    assert(!startupGate.Observe(0, true).ready);
    for (std::uint32_t sample = 2;
         sample < kWorldCandidateStableSamples;
         ++sample) {
        assert(!startupGate.Observe(0, true).ready);
    }
    auto stableUnavailable = startupGate.Observe(0, true);
    assert(stableUnavailable.ready);
    ...
    assert(!TryGetCharacterTask(
        ...
        15,
        stockTask));
    constexpr std::uintptr_t movementTask = 0x1111;
    constexpr std::uintptr_t itemUseTask = 0x2222;
    std::size_t readabilityProbes{};
    auto readableTask = [&readabilityProbes](std::uintptr_t task) {
        ++readabilityProbes;
        return task == itemUseTask;
    };
    // Ordinary movement rejects the opaque ItemUse candidate without probing
    // it. This is the task-selection hot path used by BG3WASD.
    assert(!ValidateSelectedStockTaskCandidate(
        itemUseTask, movementTask, readableTask));
    assert(readabilityProbes == 0);
    assert(!ValidateSelectedStockTaskCandidate(
        std::nullopt, movementTask, readableTask));
    assert(readabilityProbes == 0);
    // A real ItemUse selection still performs the full readability guard
    // before the redirect is allowed to inspect or replace the task.
    assert(ValidateSelectedStockTaskCandidate(
        itemUseTask, itemUseTask, readableTask));
    assert(readabilityProbes == 1);
    auto unreadableTask = [&readabilityProbes](std::uintptr_t) {
        ++readabilityProbes;
        return false;
    };
    assert(!ValidateSelectedStockTaskCandidate(
        itemUseTask, itemUseTask, unreadableTask));
    assert(readabilityProbes == 2);
